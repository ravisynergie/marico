...
<!--                <td>--><?php //echo ucfirst($tmpData['TrainingDate']);?><!--</td>-->
    ...
    public function PrintSchoolCount($tableData,$SchoolType)
    {
        ...
        ?>
        <div class="row">
            <div class="col-sm-10" style="width: 200px"><a href="javaScript:void(0)" onClick=OpenMaricoDashboard('<?php echo json_encode($StudentParameter);?>')>Back</a></div>
            <div class="col-sm-2" style="float: right">
                <img src="<?php echo base_url(); ?>assets/img/MS2.png" style="height:45px;"/><span>Model School</span>
            </div>
        </div>
	...
	public function PrintDistrictStudent($tableData)
	{
        ...
        <tbody>
			...
			{ 
				...
				$SchoolParameter=array('CallType'=>'DistrictSchoolData','PageTitle'=>'Schools','DistrctId'=>$tmpData['DistrctId']);
				?>
                <tr>
	...
	public function PrintStudent($tableData)
	{
        ...
        ?>
        <div class="row">
            <div class="col-sm-12" style="width: 200px"><a href="javaScript:void(0)" onClick=OpenMaricoDashboard('<?php echo json_encode($StudentParameter);?>')>Back</a></div>
    ...
    public function PrintTrainingHour($tableData)
    {
        ...
        <table id="MaricoTableData" class="table table-bordered">
            ...
            foreach ($tableData as $tmpData) 
            {
                if(count($NewTableData[$tmpData['SchoolId']])==0)
                {
                    $NewTableData[$tmpData['SchoolId']]=$tmpData;
                }
            }            

			foreach ($NewTableData as $tmpData) 
			{ 
				$time1 = strtotime($tmpData['InTime']);
				$time2 = strtotime($tmpData['OutTime']);
				...
				$minutes1 = ($array1[0] * 60.0 + $array1[1]);
				$minutes2 = ($array2[0] * 60.0 + $array2[1]);
				$diff =(abs(number_format(($minutes1 - $minutes2)/60, 2, '.', '')))+1 .' Hour(s)';
